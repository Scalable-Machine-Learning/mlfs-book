...
def plot_air_quality_forecast(city: str, street: str, df: pd.DataFrame, file_path: str, hindcast=False):
    ...
    # Dynamically determine the y-axis limits
    min_val = df['predicted_pm25'].min()
    if hindcast and 'pm25' in df.columns:
        min_val = min(min_val, df['pm25'].min())
    
    # Set appropriate lower limit (with some padding)
    lower_limit = max(0.1, min_val * 0.5)
    ax.set_ylim(bottom=lower_limit, top=500)
    
    # Set yticks based on the data range
    if lower_limit < 1:
        ax.set_yticks([0.1, 1, 10, 25, 50, 100, 250, 500])
    else:
    ...
    # Add legend for Air Quality Categories
    ...
    # Plot actual values if hindcast
    if hindcast == True:
